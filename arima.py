...
def arima(stock_name, data):
    ...
    fig.update_layout(title=f'{stock_name} ARIMA Data',
                      xaxis_title='Date',
    ...
    fig.update_layout(title=f'{stock_name} ARIMA Prediction',
                      xaxis_title='Date',
                      yaxis_title='Stock Price')
    st.plotly_chart(fig)
...
stock_name = st.sidebar.text_input("Stock Ticker", "ETH-USD")
...
preds, rmse = arima(stock_name, data)
